...
The skills above are the *reader-facing* slice of the replication package. The full package is the parent project folder (`AI in Social Science Research/`), which also contains:

- `background/literature/` — literature notes, classification, Zotero-compatible BibTeX, all `partially-verified` until the Stage 6 review pass.
- `asset-registry.csv` — every artifact with provenance (`human` / `agent` / `mixed`) and verification status.
- `interaction-log.csv` — every non-trivial AI-assisted session from project inception through final draft.
- `drafts/` — including `ai-agent-draft.tex`, the agent-produced first-draft snapshot of the paper.
- `tables/`, `figures/`, `scripts/`, and `background/concepts/` — inputs and derived artifacts.
...
If you use any of these skills in your own work, please cite the paper rather than the skills alone. The skills are first drafts; the paper is the argument they instantiate.
